...
function initClientUI(rows) {
  ...
  userIds = userIds.sort((a, b) =>
    usernameState[a].displayName.toLowerCase() >
    usernameState[b].displayName.toLowerCase()
      ? 1
      : -1
  );
...
function main() {
  if (document.location.search.indexOf("edit=1") >= 0) {
    ...
    if (snapshot) {
      ...
      try {
        var namespace = document.querySelector(
      ...
      if (!pageId) {
        ...
        pageId =
          document.location.origin + document.location.pathname + "?edit=1";
      }
